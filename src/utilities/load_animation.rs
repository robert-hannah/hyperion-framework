...
pub async fn load_animation(print_string: &str, connection_result: &ColoredString) {
    for _ in 0..10 {
        for &ch in &SPINNER_CHARS {
            print!("\r{print_string}{ch}"); // \r brings the cursor back to the beginning of the line
            io::stdout().flush().unwrap(); // Flush to ensure the character is printed immediately
            thread::sleep(Duration::from_millis(50)); // Pause for a short period
        }
    }

    // Clear the line after finishing and print result
    print!("\r");
