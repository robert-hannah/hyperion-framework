...
where
    T: std::fmt::Debug + Send + 'static + for<'de> Deserialize<'de> + Sync + Clone + Serialize,
{
...
async fn forwards_non_framework_messages_to_component() {
    ...
    let client_broker =
        empty_client_broker::<TestMessage>(container_state.clone(), container_state_notify.clone());
    ...
    loop {
        ...
        if tokio::time::Instant::now() > deadline {
            panic!("timeout waiting for container to close");
        }
...
async fn shutdown_and_system_shutdown_transitions_state() {
    ...
    main_tx
        .send(TestMessage::Framework(ContainerDirective::SystemShutdown))
        .await
        .unwrap();
    ...
    loop {
        ...
        if tokio::time::Instant::now() > deadline {
            panic!("timeout waiting for container to close after system shutdown");
        }
        sleep(Duration::from_millis(50)).await;
    }
